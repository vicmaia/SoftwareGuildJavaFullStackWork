...
public class LibraryDaoJdbcTemplateImpl implements LibraryDao {
    ...
    //We must use the @Transactional annotation when inserting new Publisher data 
    //into the database because we are taking two separate actions in the addPublisher 
    //method - we first insert a new row into the publishers table and then we ask MySQL 
    //for the value of the auto_increment Publisher Id assigned to the row by the database.
    @Override
    @Transactional(propagation = Propagation.REQUIRED, readOnly = false)
    ...
    //Book related interface methods
    @Override
    @Transactional(propagation = Propagation.REQUIRED, readOnly = false)
